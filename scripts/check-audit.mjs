...
/**
 * Поріг — той самий, що в `GATE-AUDIT` канону. Область — ШИРША: без
 * `--omit=dev`, тобто аудит дивиться й на інструментарій.
 *
 * Це та область, яку CI цього проєкту перевіряв і до появи обгортки, і звужувати
 * її разом із переходом на неї не можна: вразливий `eslint` чи `vite` — чужий
 * код, який виконується на машині розробника з доступом до всього репозиторію.
 * Обгортка міняє РЕАКЦІЮ на збій реєстру (§ 1.15), а не обсяг перевірки.
 */
