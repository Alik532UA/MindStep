...
const CASES = [
	...
	{
		/*
		 * ПЕРЕЛІК ПУБЛІЧНИХ КІМНАТ — той самий запит, що робить лобі:
		 * `isPrivate == false`, впорядкування за `lastActivity`, межа 50.
		 *
		 * Випадку на нього не було ЗОВСІМ, і саме він упав у продакшні: список
		 * показував «Кімнат не знайдено» при живій кімнаті, а в консолі лежало
		 * `Missing or insufficient permissions`. Тобто перевірялося все, крім
		 * запиту, на якому стоїть головний екран онлайну.
		 */
		name: 'авторизований читає перелік кімнат із межею',
		allowed: true,
		run: () => fsQuery('rooms', [{ path: 'isPrivate', raw: false }], 50, host.token)
	},
	{
		// Межа обовʼязкова: без неї один запит витягує всі кімнати з усім вмістом.
		name: 'перелік кімнат БЕЗ межі',
		allowed: false,
		run: () => fsQuery('rooms', [{ path: 'isPrivate', raw: false }], null, host.token)
	},
	{
		name: 'перелік кімнат із завеликою межею',
		allowed: false,
		run: () => fsQuery('rooms', [{ path: 'isPrivate', raw: false }], 51, host.token)
	},
	{
		/*
		 * ГОЛОВНИЙ ВИПАДОК ЦІЄЇ ТРІЙКИ: неавторизованому перелік не дається.
		 *
		 * Він і пояснює дефект продакшну. Анонімний вхід у застосунку
		 * АСИНХРОННИЙ (`authService.init` підписується на `onAuthStateChanged` і
		 * лише потім входить), а лобі питає кімнати в `onMount` — тобто запит
		 * встигає піти ДО того, як зʼявився користувач. Правила при цьому цілком
		 * справні: вони відмовляють рівно так, як і мусять.
		 */
		name: 'НЕавторизований читає перелік кімнат',
		allowed: false,
		run: () => fsQuery('rooms', [{ path: 'isPrivate', raw: false }], 50, null)
	},
	{
