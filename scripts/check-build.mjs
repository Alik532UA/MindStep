...
/*
 * Тут був виняток `NOT_APP_PAGES = new Set(["404.html"])`, і саме він зробив
 * дефект невидимим.
 *
 * Обґрунтування виняткові було правдиве для того стану: `404.html` лежав у
 * `static/`, копіювався як є й справді не був сторінкою застосунку. Але виняток
 * пережив своє обґрунтування — доки він стояв, ЖОДНА перевірка не питала, чи
 * запускає цей файл застосунок узагалі, а це рівно та властивість, якої йому
 * бракувало: шим губив адресу, і всі глибокі посилання відкривали головну.
 *
 * Тепер `404.html` пише адаптер (`fallback` у `svelte.config.js`), тобто це
 * повноцінна оболонка SPA — і вона проходить ті самі перевірки, що й головна.
 */
...
/*
 * Глибоке посилання на GitHub Pages: два файли, і кожен зі своїм статусом
 * (DEPLOY-ENVIRONMENTS-v9 § 3).
 *
 * Pages не має роутера: за невідомою адресою він віддає `404.html` і не
 * переходить нікуди, тож адреса доїжджає до клієнтського роутера цілою. Звідси
 * дві властивості, які нічим іншим не перевіряються:
 *
 *   * `404.html` мусить БУТИ й мусить бути оболонкою застосунку. Якщо його
 *     немає — Pages показує власну сторінку помилки, і жоден маршрут, крім
 *     кореня, не відкривається. Що це саме оболонка, а не будь-який HTML,
 *     доводить перевірка бутстрапа в циклі вище: файл тепер у нього входить.
 *   * `index.html` мусить бути ПЕРЕДРЕНДЕРЕНИЙ, а не позичений у фолбека.
 *     Інакше головна сторінка сайту віддається зі статусом 404 — сторінка при
 *     цьому працює, і побачити це можна лише в заголовках відповіді.
 *
 * Окремо — редирект: сáме він тут і був. `static/404.html` клав адресу в
 * `sessionStorage`, перекидав meta-refresh'ем на корінь, а повертав адресу
 * `history.replaceState` У ТОМУ Ж ФАЙЛІ — тобто на сторінці, яку refresh уже
 * зносив. Наслідок: будь-яке глибоке посилання відкривало головне меню, і
 * НІ ОДНА перевірка цього не бачила, бо `404.html` був винятком (див. початок
 * файлу). Заміряно на бойовому сайті 2026-09-14.
 *
 * Зворотний експеримент: повернути `fallback: "index.html"` — падає рядок про
 * відсутній `404.html`; додати meta-refresh у копію `build/404.html` — падає
 * рядок про редирект.
 */
{
  const shell = join(BUILD, "404.html");
  if (!existsSync(shell)) {
    fail(
      "404.html",
      "файлу немає: GitHub Pages віддасть власну сторінку помилки, і працюватиме лише корінь",
    );
  } else if (/http-equiv=["']?refresh/i.test(readFileSync(shell, "utf8"))) {
    fail(
      "404.html",
      "meta refresh: адреса, за якою прийшли, губиться — глибоке посилання відкриє головну",
    );
  }
  if (!existsSync(join(BUILD, "index.html"))) {
    fail(
      "index.html",
      "головної немає файлом — вона піде через фолбек і віддаватиметься зі статусом 404",
    );
  }
}
 ...
 * `build/` містить рівно `index.html` (передрендерений корінь) і `404.html`
 * (фолбек SPA) — обидва однакові порожні оболонки. Тег
 ...
 */
{
   ...
   */
  /*
   * CSS підняте з 23 до 30 — і це не послаблення, а наслідок того, що гейт
   * нарешті міряє СПРАВЖНЮ головну сторінку.
   *
   * Доти адаптер писав фолбек у той самий `index.html`, у який передрендер
   * клав корінь, — і фолбек ПЕРЕЗАПИСУВАВ його. Тобто `build/index.html` був
   * голою оболонкою: 10 стилів, 20.6 КБ gzip, і жодного стилю самої головної,
   * бо їх підтягував клієнтський завантажувач уже після старту JS. Гейт міряв
   * не сторінку, а каркас, і число 23 було калібруванням по ньому.
   *
   * Відколи фолбек — це `404.html` (див. `svelte.config.js`), передрендерений
   * корінь доживає до `build/`, і в ньому 18 стилів: 27.4 КБ gzip. Байти ті
   * самі, що людина качала й раніше, — змінилося лише те, чи їх видно гейту.
   * Заміряно 2026-09-14 на цій збірці, запас ~10 % як і в решти порогів.
   *
   * `CODE_BUDGET_KB` НЕ піднімається: 112.8 КБ проти 107.1 у каркасі, поріг
   * 118 витримує, і запас від того став ~5 % замість ~10 %. Це тісніше, ніж
   * задумано, і піднімати його треба не зараз, а тоді, коли він справді
   * почервоніє — із причиною, а не наперед.
   */
...
//
// ## Два документи, і форма адрес у них РІЗНА
//
// Відколи фолбек — це `404.html`, у `build/` лежать дві оболонки, і SvelteKit
// пише їм шляхи по-різному:
//
//   * `404.html` — фолбек, і він мусить піднятися з БУДЬ-ЯКОЇ глибини адреси,
//     тож шляхи абсолютні: `/MindStep/_app/…`. Для них і має сенс питання про
//     точку монтування.
//   * `index.html` — передрендерений корінь, шляхи відносні: `./_app/…`. Вони
//     сходяться з будь-якого префікса за побудовою, тож перевіряти лишається
//     тільки існування файлу.
//
// Доти вибірка брала лише абсолютні (`"(\/[^"]*\/_app\/…`), і цього вистачало,
// бо `index.html` БУВ фолбеком. Щойно він став передрендереним, та сама вибірка
// дала нуль — і замість тиші спрацювала канарка. Саме так і має поводитися
// перевірка, чий розбір застарів (AI-AGENT-PITFALLS-v9 § 1).
//
// Зворотний експеримент: підмінити хеш в одному `href` у будь-якій з оболонок —
// падає саме цей рядок із адресою й іменем файлу.
for (const shellName of ["index.html", "404.html"]) {
  const shell = join(BUILD, shellName);
  if (!existsSync(shell)) {
    fail(shellName, "оболонки немає — перевіряти нема чого");
    continue;
  }
  ...
  // Усі адреси `_app/` — і абсолютні, і відносні.
  const asked = [
    ...new Set(
  ...
  for (const url of asked) {
    ...
    if (url.startsWith("/")) {
      ...
      relative = url.slice(prefix.length);
    } else {
      // Відносна адреса рахується від самого документа, а він лежить у корені
      // `build/`; питання про префікс для неї не стоїть.
      relative = url.replace(/^\.\//, "");
    }
    const onDisk = join(BUILD, relative);
    if (!existsSync(onDisk)) {
      fail(shellName, `${url} просять, а ${norm(onDisk)} немає — 404 замість стилю чи чанка`);
    }
  }
}
