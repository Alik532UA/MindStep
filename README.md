...
Про переїзд на власний домен — [CUSTOM-DOMAIN-v9.md](../sveltekit-canon/selection_criteria/v9/ops/CUSTOM-DOMAIN-v9.md). Окремо: тут деплой іде через `peaceiris/actions-gh-pages`, який **перезаписує гілку публікації**, тож за власного домену знадобився б `static/CNAME` — інакше прив'язка злетить після першого ж деплою.
