...
const event = { url: new URL("https://example.com/MindStep/") } as never;
...
describe("handleError клієнта", () => {
  ...
  it("справжня помилка потрапляє в журнал разом зі шляхом", async () => {
    ...
    // Логер MindStep — проксі, де ім'я властивості є ТЕМОЮ, а не рівнем,
    // тож `error(...)` приймає одне повідомлення без категорії.
    const [line] = logError.mock.calls[0] as [string];
    expect(line).toContain("/MindStep/");
  ...
  it("не-Error теж переживає нормалізацію, а не падає всередині гачка", async () => {
    ...
    expect(logError.mock.calls[0]?.[0]).toContain("рядок замість Error");
  });
});
