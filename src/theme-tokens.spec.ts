...
const KNOWN_ASYMMETRY: Readonly<Record<string, readonly string[]>> = {
	/*
	 * Лишилася АСИМЕТРІЯ ЛИШЕ МЕРТВИХ ТОКЕНІВ — тих, що не має жодного споживача
	 * (див. гейт у `css-variables.spec.ts`). Оголошувати їх у парі немає сенсу:
	 * пара нічого не малює з обох боків.
	 *
	 * Живу асиметрію purple закрито 2026-08-28: чотири токени дописано в темний
	 * блок у баклажановій гамі (`--control-hover`, `--confirm-btn-hover`,
	 * `--piece-color`, `--piece-shadow`), а `--modal-content-bg` — у світлий,
...
const KNOWN_TIES: readonly string[] = [
	/*
	 * `--modal-bg` не має жодного споживача (оголошений у чотирьох місцях,
	 * спожитий нулем разів), тож нічия нічого не малює. Зникне разом із
	 * прибиранням мертвих токенів — окремою задачею.
	 *
	 * `--modal-content-bg` пішов із переліку 2026-08-28: світлий блок purple
	 * тепер оголошує його явно, і нічиї більше немає.
	 */
	'purple:--modal-bg'
];
