...
describe('хмарна база', () => {
	...
	/**
	 * ЧИТАННЯ З ПРАВИЛА — лише там, де воно НАЗВАНЕ, і лише одне на запис.
	 *
	 * `get()` у правилі — справжнє читання: воно тарифікується й обмежене 10
	 * зверненнями на запит і 20 на батч. Правило видалення ходу тягло по одному на
	 * КОЖЕН видалений документ, тож перезапуск після довгої партії впирався у
	 * правила, а не в код. Саме це й заборонено.
	 *
	 * Але «жодного читання ніде» заборонило б і те, що інакше не робиться взагалі:
	 * згоду ЧУЖОЇ людини на підписку. Її не видно ні в записі, ні в документі, який
	 * пишуть, — вона лежить у профілі того, на кого підписуються. Без читання
	 * перемикач «не підписуватися на мене» тримав би лише екран, тобто не тримав би
	 * нічого.
	 *
	 * Тому дозвіл названий поіменно й лише один: `users/{uid}/followers/{who}`. Він
	 * не має нічого спільного зі старим дефектом за формою — одне читання на ОДИН
	 * запис, який робить людина, натиснувши «підписатися», а не на кожен документ
	 * батча. Список може лише скорочуватися.
	 */
	it('правило не читає інші документи, крім названих (§ 4.8)', () => {
		/** Блоки, у яких читання з правила дозволене — з причиною в самих правилах. */
		const ALLOWED = ['/users/{uid}/followers/{who}'];

		const problems: string[] = [];
		for (const match of rulesCode.matchAll(/allow[^;]*\b(?:get|exists)\s*\(\s*\//g)) {
			// Найближчий `match` вище й є блоком, у якому стоїть це читання.
			const before = rulesCode.slice(0, match.index);
			const block = [...before.matchAll(/match\s+(\S+)\s*\{/g)].pop()?.[1] ?? '(корінь)';
			if (!ALLOWED.includes(block)) problems.push(`${block}: ${match[0]}`);
		}
		expect(problems, `читання з правила поза переліком:\n${problems.join('\n')}`).toEqual([]);

		// Перевірка, що захищає перевірку: якщо дозволений блок колись зникне разом
		// із читанням, перелік мусить скоротитися, а не лишитися простроченим.
		const stale = ALLOWED.filter((block) => !rulesCode.includes(`match ${block}`));
		expect(stale, `прострочений дозвіл на читання:\n${stale.join('\n')}`).toEqual([]);
	});
