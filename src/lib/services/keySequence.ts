 ...
 * ЗАХИСТАХ, а захист, який ніхто не перевіряє, — це побажання. Саме в цьому
 * проєкті серія `R` довго жила всередині `hotkeyService.handleKeydown` без жодного
 * з потрібних обмежень: затиснута `R` у полі пошуку витирала всі локальні дані за
 * дві секунди без запитання. Різниця не в акуратності автора, а в тому, що логіка
 * жила всередині обробника й не мала жодного тесту.
 *
