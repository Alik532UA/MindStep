 ...
 * (`npm run sync:flags`); тут пакета в залежностях НЕМА, тож оновлення набору —
 * це прогін того скрипта в сусіда й копія двох речей: `static/flags/` і
 * `countries.generated.ts`. Записано, щоб через рік це не виглядало магією.
 *
 * ## Чому назви НЕ лежать у словнику проєкту
 *
 * Двісті шістдесят дві назви × чотири мови — понад тисяча рядків, які нічим не
 * відрізняються від того, що вже є в браузері. `Intl.DisplayNames` віддає назву
