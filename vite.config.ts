	...
	icons: [
		{
			...
			type: 'image/png',
			/*
			 * 'any', а не 'maskable' — ЗАМІРЯНО, а не за смаком.
			 *
			 * `maskable` — це обіцянка, що вміст малюнка вміщується у внутрішнє
			 * коло діаметром 80% сторони: решту система має право обрізати під
			 * свою маску. У цієї іконки тло прозоре, вміст займає 78.6% площі й
			 * доходить до самого краю — радіус вмісту 96 px при полі безпеки
			 * 76.8. Тобто система обрізала б малюнок по колу.
			 *
			 * Невиконана обіцянка тут гірша за відсутню: без `maskable` система
			 * сама покладе логотип на підкладку й додасть поле.
			 *
			 * Щоб повернути `maskable`, потрібен ІНШИЙ файл: суцільне тло на весь
			 * квадрат плюс логотип у центральних 80%. Так зроблено в сусідньому
			 * `AudioRemote` (тло rgb(8,12,20), вміст 8% площі, радіус 56 при полі
			 * 77) — і там обіцянка виконується.
			 */
			purpose: 'any'
		},
		{
