...
Проект базується на сучасних стандартах **Svelte 5** та принципі "Єдиного джерела правди" (SSoT).

### Гібридна система стану
1.  **Svelte 5 Runes (`$state`, `$derived`):** Використовуються в файлах `.svelte.ts` для реалізації глибокої реактивності та чистої логіки.
2.  **Bridge Pattern:** Для забезпечення зворотної сумісності зі Svelte 4 компонентами використовуються проміжні стори (наприклад, `boardStore.ts`), які транслюють зміни з Runes у Readable/Writable стори.

### Розділення відповідальності (Logic vs Animation)
*   **Логіка (Engine/Services):** Оновлює стан миттєво. Всі ігрові розрахунки, перевірки перемоги та оновлення рахунку відбуваються синхронно або через миттєві івенти.
*   **Візуалізація (Animation Layer):** Компоненти дошки (наприклад, `GameBoard`) підписуються на зміни стану, але відображають їх з анімаціями. Це "візуальне відставання" не блокує ігрову логіку та `center-info`.
