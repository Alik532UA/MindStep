...
export default ts.config(
	...
	...svelte.configs['flat/recommended'],
	{
		...
		rules: {
			/**
			 * SVELTE-CORE-v8 § 6 — колишній головний борг цього проєкту.
			 *
			 * Було 9 знахідок: `writable`/`derived` зі `svelte/store` у семи
			 * сервісах та i18n. Це не стилістика — це два різні механізми
			 * реактивності в одному застосунку, і межа між ними проходила не по
			 * шару, а по тому, коли який файл писали.
			 *
			 * Стало 0 (2026-08-23): пʼять переведено на руни, три виявилися
			 * мертвим або write-only станом, один (`typedI18n`) лишився як
			 * законний interop зі `svelte-i18n` під точковим `eslint-disable` з
			 * причиною.
			 *
			 * `error`, а не `warn` — CODE-QUALITY-v8 § 6.4.1: правило з нульовим
			 * боргом ставиться в `error`, інакше борг відростає мовчки. Донедавна
			 * тут стояв `warn` із коментарем «9 знахідок», хоча знахідок уже не
			 * було: ратчет у `eslint-baseline.test.ts` тримав число на нулі, але
			 * сам `npm run lint` на поверненому `writable` лишався ЗЕЛЕНИМ, і
			 * побачити регресію можна було тільки прогоном тестів. Тепер її
			 * бачить перший-ліпший lint — і локально, і в CI.
			 *
